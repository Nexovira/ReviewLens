...
import { X, Mail, Lock, Store, Sparkles, ArrowRight, ShieldCheck, Check, Eye, EyeOff, KeyRound } from 'lucide-react';
import { UserProfile, PlanTier } from '../types';
import { signInUser, signUpUser, signInWithGoogleAuth, logoutUser, resetUserPassword } from '../lib/firebaseService';
      ...
      <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl relative text-slate-900">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-black text-slate-900 mt-2">
            ...
            {viewMode === 'signup' && 'Create Store Account'}
            {viewMode === 'forgot' && 'Reset Your Password'}
          </h2>
          <p className="text-xs text-slate-500 mt-1 font-medium">
            {viewMode === 'login' && 'Access your product review analytics and AI action plans.'}
            {viewMode === 'signup' && 'Start analyzing customer reviews in minutes.'}
            {viewMode === 'forgot' && 'Enter your account email to receive a password reset link.'}
          </p>
        </div>
          ...
          >
            {isLoading ? (
              ...
              <span>
                ...
                {viewMode === 'forgot' && 'Send Password Reset Link'}
              </span>
            )}
          </button>
        </form>
